...
let listSum = l => List.fold_left((sum, i) => sum + i, 0, l);
...
let part2 = lines => {
  ...
  let rec getWinner = (game, debugLen, debugSum, p1, p2, v) => {
    ...
    // Some debugging to find errors.
    if (List.length(p1) + List.length(p2) !== debugLen) {
      failwith("Lengths changed!");
    };

    // Some debugging to find errors.
    if (listSum(p1) + listSum(p2) !== debugSum) {
      ...
      if (CamlList.isEmpty(p1)) {
        ...
        let res =
          if (List.length(p1) < f1 || List.length(p2) < f2) {
            ...
            if (f1 > f2) {
              getWinner(game, debugLen, debugSum, p1 @ [f1, f2], p2, v);
            } else {
              getWinner(game, debugLen, debugSum, p1, p2 @ [f2, f1], v);
            };
          } else {
